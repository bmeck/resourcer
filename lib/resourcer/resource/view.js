...
var render = exports.render = function (template, attributes) {
...
this.filter = function (name /* [options], filter */) {
    ...
    //
    // Namespace the views on the Resource prototype. This is to avoid side effects on 
    // sibling resources which all utilize resourcer.Resource.prototype
    //
    R.views[R.resource] = R.views[R.resource] || {};
    
    if (R.connection.protocol === 'database') { // Only CouchDB for now
        R._design = R._design || {
            _id:   ['_design', R.resource].join('/'),
            views: R.views[R.resource] || {}
        };
